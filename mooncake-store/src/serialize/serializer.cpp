...
tl::expected<void, SerializationError> Serializer<MountedSegment>::serialize(
    ...
    if (mounted_segment.buf_allocator) {
        ...
        if (offsetAllocator) {
            ...
            packer.pack(mounted_segment.segment.RackId());
            return {};
        }
    }
...
Serializer<MountedSegment>::deserialize(const msgpack::object &obj) {
    ...
    try {
        ...
        // Snapshots written before rack affinity stop at host_id. Leaving
        // rack_id empty there is correct: such a segment predates any rack
        // identity, and an empty rack_id keeps it out of the rack index (and
        // therefore out of strict-rack placement) rather than silently
        // claiming membership in whatever rack the reader happens to be in.
        if (obj.via.array.size >= 10) {
            mounted_segment.segment.rack_id = array[9].as<std::string>();
        }
    } catch (const std::exception &e) {
        return tl::unexpected(SerializationError(
            ErrorCode::DESERIALIZE_FAIL,
